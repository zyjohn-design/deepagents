#!/usr/bin/env python3
"""
活动文档解析工具 v3 (基于 LLM + Instructor)

核心优化：
1. 两阶段流水线：Phase 1 结构识别 → Phase 2 统一提取（区域+任务同时提取）
2. 每 chunk 仅 1 次 LLM 调用（旧版需要 2 次：区域+任务分开）
3. 精简 prompt：只注入枚举定义，不注入完整知识库原文
4. 增大 chunk size（6000 chars），减少分块数
5. 管控区域按 (area_name, phase) 去重 + boundaries 模糊匹配合并
6. Literal 类型锁死知识库枚举，Pydantic 层面拦截非法值

性能对比（3 chunks 文档）：
  v2: 1(Phase1) + 3(Phase2) + 3(Phase3) = 7 次 LLM 调用
  v3: 1(Phase1) + 2(Phase2, chunk更大) = 3 次 LLM 调用
...
class EventDocExtractor:
    ...
    def _init_dynamic_models(self):
        ...
        # --- 联合模型：单次调用同时提取区域+任务 ---
        self.DynChunkOutput = create_model(
            'ChunkExtractionOutput',
            affected_areas=(List[self.DynAreaItem], Field(
                default_factory=list,
                description="本片段涉及的管控区域。同一区域在不同阶段有不同措施时输出多条"
            )),
            tasks=(List[self.DynTaskItem], Field(
                default_factory=list,
                description="本片段的管控任务列表，按时间顺序"
            )),
            __base__=BaseModel,
        )

    def _build_compact_knowledge(self) -> str:
        """构建精简版知识摘要，只包含枚举定义和关键规则。

        完整知识库 3000+ 字符，精简版 ~500 字符，大幅减少 token。
        """
        parts = []
        parts.append("【阶段枚举】")
        ...
        parts.append("【区域枚举】")
        if self.has_strict_area_names:
            parts.append(f"  area_name 只能是: {self.area_names_enum}")
        else:
            parts.append("  area_name: 从文档提取具体名称")
        parts.append(f"  type 只能是: {self.area_types_enum}")

        # 提取关键术语（如果有）
        term_pattern = r'### (?:管控措施术语|专业术语表|赛事术语).*?\n\|.*?\n\|.*?\n((?:\|.*?\n)*)'
        match = re.search(term_pattern, self.knowledge_base, re.DOTALL)
        if match:
            parts.append("【关键术语】")
...
- 用简短的话概括每个阶段的内容"""
    ...
    # ------------------------------------------------------------------
    # Phase 2: 统一提取（区域 + 任务，每 chunk 一次调用）
    # ------------------------------------------------------------------

    def phase2_unified_extract(
        self,
        chunks: List[str],
        structure: PhaseDetectionOutput
    ) -> Tuple[List[dict], List[dict]]:
        """每个 chunk 单次 LLM 调用，同时提取区域和任务。

        相比分离的 Phase 2 + Phase 3（每 chunk 调用 2 次），
        统一提取将 LLM 调用次数减半：N chunks → N 次调用（而非 2N 次）。
        """
        phases_context = "\n".join(
        ...
        # 构建区域约束描述
        if self.has_strict_area_names:
            area_constraint = f"""- area_name 只能是: {self.area_names_enum}
- type 只能是: {self.area_types_enum}"""
        else:
            area_constraint = f"""- area_name: 从文档提取具体名称
- type 只能是: {self.area_types_enum}"""

        compact_knowledge = self._build_compact_knowledge()

        base_system_prompt = f"""你是交通管控文档提取专家。请从文档片段中同时提取【管控区域】和【管控任务】。
...
**affected_areas（管控区域）**:
1. 同一区域在不同阶段有不同措施时，输出多条记录
2. 贯穿全程的管控（应急通道、医疗保障等）phase 填"全时段保障"
3. boundaries 保留原文路名

**tasks（管控任务）**:
1. 穷尽提取所有管控动作、时间节点
2. 每个独立管控动作单独一条
3. affected_area 必须引用 affected_areas 中已定义的 area_name
4. 时间格式: YYYY-MM-DD HH:MM
5. 跨年处理: 12月31日之后的凌晨时段推断为下一年
...
### 场景: {self.scene_type} | 活动日期: {self.event_date or '未指定'}"""
        ...
        for i, chunk in enumerate(chunks):
            ...
            if self.examples:
                # 截取到第一个 ## 示例2 之前
                first_example_end = self.examples.find("## 示例2")
                example_text = self.examples[:first_example_end] if first_example_end > 0 else self.examples[:800]
                messages.append({"role": "user", "content": "提取示例："})
                messages.append({"role": "assistant", "content": example_text})

            messages.append({"role": "user", "content": f"请提取以下文档片段的管控区域和任务：\n\n{chunk}"})

            try:
                result = self.client.chat.completions.create(
                    model=self.model_name,
                    response_model=self.DynChunkOutput,
                    messages=messages,
                    temperature=0.1,
                    max_retries=3,
                )
                ...
                # 处理区域
                for area in (result.affected_areas or []):
